...
export class ItemWideService {
  ...
  public getItems(limit: number = 50, offset: number = 0,
                  item_type: string | null = null): Observable<ItemWideI[]> {
  ...
  public deleteItem(itemId: number): void {
    this.httpClient.delete(this.apiUrl + String(itemId));
  }
  ...
  public putItem(itemId: number, itemData: ItemWideI): Observable<ItemWideI> {
    return this.httpClient.put<ItemWideI>(this.apiUrl + String(itemId), itemData);
  }
}
