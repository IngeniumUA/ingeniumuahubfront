...
export class PopupzComponent {
  ...
  ngOnInit() {
    ...
    this.getProducts();
    this.httpClient.get<EventItemDetailI>(apiEnviroment.apiEnv['apiUrl'] + "popup/event").pipe(shareReplay()).subscribe((item) => {
      this.event = item;
    });
  }
  ...
  getProducts(): void {
    this.products$ = this.httpClient.get<IProductItem[]>(apiEnviroment.apiEnv + "popup/" + this.category);
  }
