...
export class SettingsPage {
  ...
  public ionViewWillEnter() {
    this.storage.get("settings")?.then((result: any) => {
      if (result !== undefined) {
  ...
  public setAutoReturn() {
    ...
    this.storage.set("settings", {"autoReturn": this.returnToggle.checked})
  }

  public setDisableSound() {
    disableSound = this.soundToggle.checked
    this.storage.set("settings", {"soundDisable": this.soundToggle.checked})
  }
  ...
  public setReturnTime() {
    ...
    if (!isNaN(+this.inputReturnTime)) {
      ...
      this.storage.set("settings", {"returnTime": this.inputReturnTime})
    } else {
      this.showError = true
  ...
  public clearSettings(event: any) {
    if (event.detail["data"] === undefined) {return}
    if (event.detail["data"]["action"] === "delete") {
      this.storage.clearSpecific("settings")
    }
  }
