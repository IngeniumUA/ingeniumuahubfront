<script lang="ts">
	 ...
	 */
	let { data } = $props();
	let vacatures = $state(data.vacatures);
	let total_vacatures_count: number = $state(data.total_vacatures_count);
	let available_vacatures_count: number = $state(data.available_vacatures_count);

	/**
	 * Refreshing data
	 */
	let onlyShowActive: boolean = $state(true);
	 ...
	 * Edit modal and form
	 */
	let editItemSelectedIndex: null | number = $state(null);
	let editItemSelected: PromoItemWideI | null = $state(null);
	let editItemExternalLink: boolean = $state(false);

	let itemPutError: string | null = $state(null);
	let loadingHTTP: boolean = $state(false);

	function setEditItemIndex(index: number | null) {
		itemPutError = null;
		editItemSelectedIndex = index;
		if (editItemSelectedIndex !== null && editItemSelectedIndex < vacatures.length) {
			editItemSelected = vacatures.at(editItemSelectedIndex)!;
	...
	let editItemImage = $derived.by(() => editItemSelected
		? editItemSelected.derived_type.display.image_square ?? editItemSelected.derived_type.display.image_landscape
		: null)
	let recsysPreview = $derived.by(() => editItemSelectedIndex !== null && editItemSelected
		? {
	...
	<div class="container flex flex-col md:flex-row">
		...
		<div class="md:w-2/3">
			...
			<div class="flex justify-between items-center mb-6">
				...
				<label class="inline-flex items-center cursor-pointer my-4">
					<input type="checkbox" bind:checked={onlyShowActive} onclick={() => refreshTable()} class="sr-only peer">
					<div class="
							relative w-11 h-6
		...
		<div class="md:w-1/3">
			...
			<div class="p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow">
				...
				<p class="text-blue-900 font-bold">{available_vacatures_count}</p>
			</div>

			<div class="p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow">
				<h4 class="text-ingenium-grey-800 font-bold">Aantal Totaal:</h4>
				<p class="text-blue-900 font-bold">{total_vacatures_count}</p>
			</div>
...
{#if editItemSelectedIndex !== null && editItemSelected !== null}
	<div
		...
		role="button"
		tabindex="0"
		onkeydown={(e) => {if (e.key === 'Escape') {setEditItemIndex(null);}}}
	>
		<div
		...
		>
			...
			<div class="flex justify-between p-2">
				...
				<form class="flex-1 ingenium-form"
							onsubmit="{(e) => {e.preventDefault(); handlePut()}}">
					<fieldset>
						...
						<div class="flex-1">
							...
							<fieldset>
								<div class="form-field">
									...
									<input id="vacatureImage" type="text" required value={ editItemImage }/>

									<label class="inline-flex items-center cursor-pointer my-4">
										<input type="checkbox"
													 checked={editItemSelected.derived_type.display.image_square !== null}
													 onchange={(e) => handleImageToggle(e.currentTarget.checked)} class="hidden peer">
			...
			<div class="p-2 flex justify-between items-center border-t dark:border-gray-600 border-gray-200">
				...
				{#if (editItemSelected.item.availability.available)}
					<button class="button button-danger w-24 button-inline"
									disabled={loadingHTTP}
									onclick={() => {handleAvailableButton(false)}}
					>
				...
				{:else}
					<button class="button-success button w-24 button-inline"
									disabled={loadingHTTP}
									onclick={() => {handleAvailableButton(true)}}>
...
<AddNewItem bind:isOpen={ showAddingNew } itemType="promoitem"></AddNewItem>